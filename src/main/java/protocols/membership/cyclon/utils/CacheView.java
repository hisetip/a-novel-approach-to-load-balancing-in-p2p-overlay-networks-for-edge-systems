...
public class CacheView {
    ...
    public boolean addPeer(Host peer, int age) {
        if(!peer.equals(self)) {
            if (!cache.containsKey(peer) && cache.size() >= capacity) { // TODO If there are bugs with this, change >= with ==
                dropRandom();
            }
    ...
    public void merge(Map<Host, Integer> subset, Set<Host> sent) {
        ...
        dup.forEach((h, a) -> {
            if(cache.size() >= capacity) { // TODO If there are bugs with this, change >= with ==
