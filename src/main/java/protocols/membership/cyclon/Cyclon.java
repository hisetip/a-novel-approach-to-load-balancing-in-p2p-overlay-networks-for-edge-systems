...
import protocols.membership.cyclon.messages.ShuffleMessage;
import protocols.membership.cyclon.messages.ShuffleReplyMessage;
import protocols.membership.cyclon.requests.MembershipReply;
import protocols.membership.cyclon.requests.MembershipRequest;
import protocols.membership.cyclon.timers.ShuffleTimer;
import protocols.membership.cyclon.utils.CacheView;
...
public class Cyclon extends GenericProtocol {
    ...
    public final static short PROTOCOL_ID = 14000;
    ...
    private Set<Host> lastSentHosts;
    ...
    public Cyclon(String channelName, Properties properties, Host myself) throws IOException, HandlerRegistrationException {
        ...
        int capacity = Integer.parseInt(properties.getProperty("cacheView", "7")); //param: capacity of cacheView
        this.shuffleTime = Short.parseShort(properties.getProperty("shuffleTime", "2000")); //param: timeout for shuffle
        subsetSize = Integer.parseInt(properties.getProperty("shuffleLen", "4"));
        ...
        this.lastSentHosts = new HashSet<>();
    ...
    private void uponRequestNumberOfNeighborsNotification(RequestedNumberOfNeighborsNotification notification, short sourceProto) {
        logger.debug("Received new neighbor request {}, current cache size is {}", notification.getRequestedNum(), this.cacheView.getCache().size());
        if (notification.getRequestedNum() < this.cacheView.getCache().size()) {
            trimNeighbors(this.cacheView.getCache().size() - notification.getRequestedNum());
        }

        this.cacheView.setCapacity(notification.getRequestedNum());
    }

    // Aux method for trimming neighbors after receiving a requestNumberOfNeighborsNotification
    private void trimNeighbors(int toTrim) {
        int newNeighLength = this.cacheView.getCache().size() - toTrim;
    ...
    private void uponShuffleTime(ShuffleTimer timer, long timerId) {
        ...
        Set<Host> newLastSent = new HashSet<>();
        for (Host previousHost : lastSentHosts) {
            if(!cacheView.getCache().contains(previousHost)) {
                triggerNotification(new NeighborDown(previousHost));
            } else {
                newLastSent.add(previousHost);
            }
        }

        for (Host currentHost : cacheView.getCache()) {
            if(!lastSentHosts.contains(currentHost)) {
                triggerNotification(new NeighborUp(currentHost));
                newLastSent.add(currentHost);
            }
        }

        lastSentHosts = newLastSent;
    }
