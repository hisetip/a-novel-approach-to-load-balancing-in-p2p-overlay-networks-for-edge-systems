package protocols.membership.hyparviewresest;
...
import protocols.membership.hyparviewresest.messages.*;
import protocols.membership.hyparviewresest.timers.HelloTimeout;
import protocols.membership.hyparviewresest.timers.ShuffleTimer;
import protocols.membership.hyparviewresest.utils.IView;
import protocols.membership.hyparviewresest.utils.View;
...
public class HyparViewResEst extends GenericProtocol {

    private static final Logger logger = LogManager.getLogger(HyparViewResEst.class);

    public final static short PROTOCOL_ID = 12000;
    public final static String PROTOCOL_NAME = "HyParViewResEst";
    ...
    public HyparViewResEst(String channelName, Properties properties, Host myself) throws IOException, HandlerRegistrationException {
        ...
        int maxPassive = Integer.parseInt(properties.getProperty("PassiveView", "25")); //param: maximum passive nodes
    ...
    protected void handleDropFromActive(Host dropped) {
        if(dropped != null) {
            triggerNotification(new NeighborDownResEst(dropped, (short) -1, (short)-1));
    ...
    private void uponReceiveJoin(JoinMessage msg, Host from, short sourceProto, int channelId) {
        ...
        triggerNotification(new NeighborUpResEst(from, (short)-1, (short)-1));
    ...
    private void uponReceiveForwardJoin(ForwardJoinMessage msg, Host from, short sourceProto, int channelId) {
        ...
        if(msg.decrementTtl() == 0 || active.getPeers().size() == 1) {
            if(!msg.getNewHost().equals(myself) && !active.containsPeer(msg.getNewHost())) {
                ...
                triggerNotification(new NeighborUpResEst(msg.getNewHost(), (short) -1, (short) -1));
    ...
    private void uponOutConnectionDown(OutConnectionDown event, int channelId) {
        ...
        if(active.removePeer(event.getNode())) {
            triggerNotification(new NeighborDownResEst(event.getNode(), (short)-1, (short)-1));
            if(!active.fullWithPending(pending)){
                setupTimer(new HelloTimeout(), timeout);
            }
        } else
