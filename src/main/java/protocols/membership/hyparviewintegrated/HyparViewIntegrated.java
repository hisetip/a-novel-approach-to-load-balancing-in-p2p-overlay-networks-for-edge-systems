package protocols.membership.hyparviewintegrated;
...
import protocols.membership.hyparviewintegrated.messages.*;
import protocols.membership.hyparviewintegrated.timers.HelloTimeout;
import protocols.membership.hyparviewintegrated.timers.ShuffleTimer;
import protocols.membership.hyparviewintegrated.utils.IView;
import protocols.membership.hyparviewintegrated.utils.View;
...
public class HyparViewIntegrated extends GenericProtocol {

    private static final Logger logger = LogManager.getLogger(HyparViewIntegrated.class);

    //it has the same ID as hyparview but that's ok since they'll never run at the same time
    public final static short PROTOCOL_ID = 16000;
    public final static String PROTOCOL_NAME = "HyParViewIntegrated";
    ...
    public HyparViewIntegrated(String channelName, Properties properties, Host myself) throws IOException, HandlerRegistrationException {
    ...
    protected void handleDropFromActive(Host dropped) {
        if(dropped != null) {
            ...
            triggerNotification(new NeighborUpResEst(dropped));
    ...
    private void uponReceiveJoinReply(JoinReplyMessage msg, Host from, short sourceProto, int channelId) {
        ...
        if(!active.containsPeer(from)) {
            ...
            triggerNotification(new NeighborDownResEst(from));
            pending.remove(from);
    ...
    private void uponReceiveForwardJoin(ForwardJoinMessage msg, Host from, short sourceProto, int channelId) {
        ...
        if(msg.decrementTtl() == 0 || active.getPeers().size() == 1) {
            if(!msg.getNewHost().equals(myself) && !active.containsPeer(msg.getNewHost())) {
                ...
                triggerNotification(new NeighborDownResEst(msg.getNewHost()));
            ...
            if(msg.getTtl() == PRWL)  {
                ...
                triggerNotification(new NeighborUpResEst(msg.getNewHost()));
    ...
    private void uponReceiveHelloReply(HelloReplyMessage msg, Host from, short sourceProto, int channelId) {
        ...
        if(msg.isTrue()) {
            ...
            triggerNotification(new NeighborUpResEst(from));
        ...
        if(msg.decrementTtl() > 0 && active.getPeers().size() > 1) {
            ...
            for (Host host : msg.getFullSample()) {
                if (!host.equals(myself) && !active.containsPeer(host) && passive.isFull() && i < peers.size()) {
                    ...
                    triggerNotification(new NeighborDownResEst(hosts[i]));
                    i++;
                }
                passive.addPeer(host);
                triggerNotification(new NeighborUpResEst(host));
            }
    ...
    private void uponReceiveShuffleReply(ShuffleReplyMessage msg, Host from, short sourceProto, int channelId) {
        ...
        for (Host h : sample) {
            if(!h.equals(myself) && !active.containsPeer(h) && passive.isFull() && i < sent.length) {
                ...
                triggerNotification(new NeighborDownResEst(sent[i]));
                i ++;
            }
            passive.addPeer(h);
            triggerNotification(new NeighborUpResEst(h));
        }
        logger.trace("After Passive{}", passive);
    }
