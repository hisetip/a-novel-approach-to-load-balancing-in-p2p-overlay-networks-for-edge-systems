package protocols.membership.cyclonresest;
...
import protocols.membership.cyclonresest.messages.ShuffleMessage;
import protocols.membership.cyclonresest.messages.ShuffleReplyMessage;
import protocols.membership.cyclonresest.requests.MembershipReply;
import protocols.membership.cyclonresest.requests.MembershipRequest;
import protocols.membership.cyclonresest.timers.ShuffleTimer;
import protocols.membership.cyclonresest.utils.CacheView;
...
public class CyclonResEst extends GenericProtocol {

    private static final Logger logger = LogManager.getLogger(CyclonResEst.class);

    public final static short PROTOCOL_ID = 15000;
    public final static String PROTOCOL_NAME = "CyclonResEst";
    ...
    public CyclonResEst(String channelName, Properties properties, Host myself) throws IOException, HandlerRegistrationException {
    ...
    private void uponShuffleTime(ShuffleTimer timer, long timerId) {
        ...
        for (Host previousHost : lastSentHosts) {
            if(!cacheView.getCache().contains(previousHost)) {
                triggerNotification(new NeighborDownResEst(previousHost));
            } else {
        ...
        for (Host currentHost : cacheView.getCache()) {
            if(!lastSentHosts.contains(currentHost)) {
                triggerNotification(new NeighborUpResEst(currentHost));
